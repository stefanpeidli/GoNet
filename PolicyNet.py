...
class PolicyNet:
    ...
    def Learnpropagate(self,eta ,testdata , targ):
        for epoch in range(0,games):
            ...
            self.errorsbyepoch[epoch]=self.compute_ms_error (y[:-1], targ)
            self.abserrorbyepoch[epoch]=self.compute_error (y[:-1], targ)
            self.KLdbyepoch[epoch]=self.compute_KL_divergence(y[:-1], targ)
...
eta=0.001
[firstout,out]=NN.Learnpropagate(eta ,testdata , targ)
